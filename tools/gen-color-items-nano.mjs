#!/usr/bin/env node
// Module-5 (what-color) outline generator using Google Gemini 2.5 Flash Image.
// Only 6 items are generated — colors are applied later via CSS masking on
// the flood-filled silhouette produced by optimize-scenes.
//
// Usage:
//   npm run gen-color-items-nano
//   npm run gen-color-items-nano -- --only ball,box
//   npm run gen-color-items-nano -- --force
...
const itemsMod = await import(pathToFileURL(path.join(root, 'src/data/color-items.js')).href);
const allItems = itemsMod.colorItems;
const onlySet = ONLY ? new Set(ONLY.split(',')) : null;

const outDir = path.join(root, 'public/assets/scenes-5/items');
